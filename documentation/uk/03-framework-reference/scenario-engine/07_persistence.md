...
| Подія | Час персистентності | Режим довговічності | Обґрунтування |
|-------|---------------|-----------------|-----------|
| `on_scenario_started` (LOADED→RUNNING) | **Негайно** | `CrashCritical` | Критична для збоїв подія має пережити; емітиться поза керуючим циклом (start() — у HTTP-задачі) |
| `on_scenario_terminal` (COMPLETED, FAILED, включно з ABORTING→FAILED) | **Негайно** | `CrashCritical` | Фінальний стан має бути зафіксований; раз на запуск |
| `on_phase_entered` для головної доріжки (`MODR_TRACK_FLAG_MAIN`) | **Негайно** | `Deferred` | Інваріанти головної доріжки збережено; запис у черзі поза циклом |
| `on_phase_entered` для бічної доріжки | Дроселюється до ≥1 с між записами | `Deferred` | Захист від зношування flash; запис у черзі поза циклом |
| 5-хвилинна контрольна точка (Stage 1.5) | Періодично | `Deferred` | Гарантує точність resume, якщо активна лише бічна доріжка |

### Довговічність запису поза циклом (`NvsWriteMode`)

Крайові події зміни фази детектуються *всередині* такту engine (100 Гц).
Синхронний `nvs_set_blob` + `nvs_commit` там (десятки мс flash-I/O) перевищує
бюджет такту 10 мс і стопорить керуючий цикл. Щоб цього уникнути, observer
більше не вирішує, *де* виконується запис, — він лише позначає кожен запис
**режимом** довговічності, а цільовий колбек його дотримується:

- **`Deferred`** — записи зміни фази. Колбек кладе серіалізований токен у чергу
  FreeRTOS і одразу повертається; виділена низькопріоритетна задача спорожнює
  чергу й виконує власне запис у flash. Такт ніколи не блокується на flash.
  Втрата найсвіжішого deferred-токена при збої лише відкочує відновлення на одну
  фазу (відновлення все одно входить у PAUSED — без актуації).
- **`CrashCritical`** — старт / термінал сценарію. Колбек кладе токен у чергу
  **і блокується, доки задача-споживач його не закомітить**. Оскільки є одна
  задача-споживач і черга FIFO, до моменту коміту crash-critical елемента всі
  попередні deferred-записи цього слоту вже скинуто — тож застарілий токен фази
  ніколи не перезапише термінальний токен, а токен довговічний до повернення з
  обробника події.

Саме ця схема (один писар + FIFO) зберігає семантику відновлення після збою,
водночас виносячи часті записи з циклу. Референсна реалізація — у
`main/scenario_persist.{h,cpp}` (черга FreeRTOS + задача-споживач); якщо
виділення черги не вдалося, відбувається відкат до синхронного прямого запису
...
```cpp
// `mode` — підказка довговічності (Deferred / CrashCritical); див. розділ
// «Довговічність запису поза циклом» вище.
...
Підключення на цільовій платформі у `main.cpp` — колбеки надає модуль
`scenario_persist` (черга FreeRTOS + низькопріоритетна задача-споживач):

```cpp
// Створює чергу, примітиви скидання та задачу-споживач. Викликати один раз до
// того, як engine почне виконувати сценарії. У разі збою — відкат до синхронних
// записів.
modesp::scenario_persist::init();

static modesp::scenario::NvsObserver nvs_obs{
    &modesp::scenario_persist::write,   // дотримується NvsWriteMode (defer / flush)
    &modesp::scenario_persist::read,    // синхронне читання NVS (відновлення на старті)
    nullptr};
```

`scenario_persist::write` адресує слот у `scnstate/t<slot>` точно як раніше;
змінюється лише те, *де* виконується запис у flash. Реалізацію черги /
задачі-споживача див. у `main/scenario_persist.{h,cpp}`.

Колбек запису викликається з двох задач — `on_scenario_started` із HTTP-задачі
(через `start()`), краї фази/терміналу із задачі оновлення engine — тож має
бути потокобезпечним. Бекенд `scenario_persist` це забезпечує: `xQueueSend`
безпечний для кількох виробників, а м'ютекс серіалізує рукостискання скидання
crash-critical, щоб виробники не крали сигнали завершення один в одного. Єдина
задача-споживач — єдиний код, що торкається NVS, тож записи ніколи не
переставляються.
