...
// ─────────────────────────────────────────────────────────────────
// Scenario engine API handlers (Step 16b)
// ─────────────────────────────────────────────────────────────────
//
// REST surface для SequenceEngine:
...
bool extract_handle(const char* buf, int len, modesp::sequence::SequenceHandle& out) {
    jsmn_parser p;
    jsmntok_t tokens[16];
    jsmn_init(&p);
    ...
    for (int i = 1; i + 1 < r; i += 2) {
        ...
        if (klen == 6 && std::strncmp(buf + tokens[i].start, "handle", 6) == 0) {
            ...
            out = static_cast<modesp::sequence::SequenceHandle>(v);
            return true;
        }
    }
    return false;
...
const char* engine_error_str(modesp::sequence::EngineError e) {
    using EE = modesp::sequence::EngineError;
    switch (e) {
...
const char* state_str(modesp::sequence::SequenceRuntime::State s) {
    using SS = modesp::sequence::SequenceRuntime::State;
    switch (s) {
...
void send_error_json(httpd_req_t* req, modesp::sequence::EngineError e) {
    char body[96];
...
bool require_engine(httpd_req_t* req, modesp::sequence::SequenceEngine*& out) {
    auto* self = static_cast<HttpService*>(req->user_ctx);
    if (!self || !self->server()) return false;  // shouldn't happen
    // Engine pointer access via friend-style helper. Since set_sequence_engine
    // stores у private member, we expose через а getter-pattern below.
    out = self->sequence_engine();
    if (!out) {
...
esp_err_t HttpService::handle_get_scenario_list(httpd_req_t* req) {
    if (!check_auth(req)) return ESP_OK;
    modesp::sequence::SequenceEngine* eng = nullptr;
    ...
    int n = std::snprintf(buf, sizeof(buf), "{\"active\":%u,\"slots\":[",
                          eng->active_count());
    bool first = true;
    for (uint8_t i = 1; i <= modesp::sequence::MAX_SEQUENCES; ++i) {
        auto state = eng->state(i);
        if (state == modesp::sequence::SequenceRuntime::State::IDLE) continue;
...
esp_err_t HttpService::handle_get_scenario_info(httpd_req_t* req) {
    ...
    auto h = static_cast<modesp::sequence::SequenceHandle>(handle);
    ...
    for (uint8_t t = 0; t < tc; ++t) {
        ...
        switch (eng->track_state(h, t)) {
            case modesp::sequence::TrackRuntime::State::IDLE:      ts = "idle"; break;
            case modesp::sequence::TrackRuntime::State::RUNNING:   ts = "running"; break;
            case modesp::sequence::TrackRuntime::State::WAITING_FOR_RESOURCE: ts = "waiting"; break;
            case modesp::sequence::TrackRuntime::State::ABORTING:  ts = "aborting"; break;
            case modesp::sequence::TrackRuntime::State::COMPLETED: ts = "completed"; break;
            case modesp::sequence::TrackRuntime::State::FAILED:    ts = "failed"; break;
        }
        n += std::snprintf(buf + n, sizeof(buf) - n,
...
esp_err_t handle_post_lifecycle(httpd_req_t* req,
                                modesp::sequence::EngineError (*op)(
                                    modesp::sequence::SequenceEngine*,
                                    modesp::sequence::SequenceHandle)) {
    ...
    body[len] = '\0';
    modesp::sequence::SequenceHandle h = 0;
    if (!extract_handle(body, len, h)) {
        httpd_resp_send_err(req, HTTPD_400_BAD_REQUEST, "Missing handle");
        return ESP_FAIL;
    }
    auto err = op(eng, h);
    if (err != modesp::sequence::EngineError::OK) {
...
esp_err_t HttpService::handle_post_scenario_start(httpd_req_t* req) {
    return handle_post_lifecycle(req,
        [](modesp::sequence::SequenceEngine* e, modesp::sequence::SequenceHandle h) {
            return e->start(h);
        });
}
