...
> 📖 **Українською:** [documentation/uk/03-framework-reference/scenario-engine/usage/examples/02_dual_track_sync.md](../../../../../uk/03-framework-reference/scenario-engine/usage/examples/02_dual_track_sync.md)

Two parallel tracks coordinating through SharedState. Demonstrates ADR-0003
tick-order semantics: track 0's writes within a tick are visible to track 1's
reads in the same tick (declaration order matters).
...
Below are the relevant fragments (full file: ~100 lines).
...
```jsonc
{
  ...
  "phases": [
    ...
    },
    {
      ...
      "transitions": [
        // Either an input drives the transition fast (via all_of), or 5s timer fallback
...
The watcher reads the engine-written mirror key `abs_test.main_phase_name` —
NOT a user-written key. The engine updates mirror keys on phase advance,
making them publish-subscribe channels for track coordination.

## Tick-order semantics (ADR-0003)

The engine ticks tracks in declaration order:
...
Within a tick, track 1's read happens AFTER track 0's write. The engine reads
SharedState fresh each time, no snapshot. This matches developer intuition:
"the next read after a write sees the new value".

**Implication:** declare tracks in producer-then-consumer order. If the watcher
were declared first, it would read STALE main_phase_name on tick N (before
main updated it) and wait one tick for convergence — usually harmless but
adds latency.

## Verification

Cross-track sync is verified end-to-end via a host test:

```bash
python -m pytest tools/tests/test_sequence_host.py::test_track_synchronization_host
```

The test loads a minimal 2-track variant (sync_two_tracks.modr) and drives
...
## Adding a safety abort (global transition)

To force-abort the scenario regardless of which phase tracks are in, use a
global transition:
...
Global transitions evaluate FIRST each tick, in priority order (descending).
First match aborts the scenario (all tracks → FAILED → scenario FAILED).
...
A monitor track that runs in parallel with main, fails on safety violation:
...
With `completion_rule: main_track_complete`, the watchdog's $abort triggers
scenario fail; main's $complete triggers scenario success.

### Producer-consumer pipeline

Track 0 produces a work item count; Track 1 consumes by waiting on count:
