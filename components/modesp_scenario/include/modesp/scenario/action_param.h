 ...
 * Caller-friendly з будь-якого C++ модуля без modesp_scenario header
 * dependency cascade.
 *
 * Wire format: matches `modr_param_entry` у modr_format.h byte-for-byte.
 *
 * Lift from `modesp_sequence/action_param.h` (Phase 1): namespace rename +
 * SharedState* swap for IStateBackend* — actions now write to injected
 * backend, not directly to SharedState (decouples engine з concrete state).
 */
...
class IStateBackend;  // forward — full include not required для pointer
...
/// Engine populates на кожен call; action reads, optionally writes via state.
