...
> 📖 **Українською:** [documentation/uk/03-framework-reference/scenario-engine/usage/03_registering_actions.md](../../../../uk/03-framework-reference/scenario-engine/usage/03_registering_actions.md)

ModESP business modules that want to be controllable by scenarios add domain-
specific actions and conditions to the ActionRegistry. The built-in vocabulary
(`log`, `set_state`, `wait_ms` + 10 leaf conditions) is intentionally
hardware-agnostic; actual hardware control lives in domain modules.
...
All action and condition handlers share the signature:
...
The recipe author calls the action with named params:

```jsonc
{"action": "mc.set_target_temp", "params": {"temp": 85.5}}
```

compile_scenario.py packs this into ActionParam[] with key_hash = djb2("temp").
The action handler looks it up by hash:
...
ActionStatus MulticookerModule::action_set_target_temp(
    ...
    // Find param by name hash. Linear scan on short arrays — params are typically
    // 1-3, faster than hashmap lookup for this size.
    ...
    if (temp < 0.0f || temp > 200.0f) {
        // Out-of-range value — recoverable; log + continue with transitions
        ESP_LOGW("mc", "target_temp %.1f° out of range", temp);
        return ActionStatus::FAILED_RECOVERABLE;
    }

    // Write target to SharedState — the actual hardware module reads this and drives
...
String params store an offset into the recipe's string pool. Use the helper:

```cpp
char keybuf[32];
const auto* p_key = /* find param with hash djb2("key"), type STR */;
uint16_t off = p_key->v.s_idx;
// Manually walk the pool: byte at offset = length, followed by raw bytes
...
keybuf[len] = '\0';
// keybuf is now a null-terminated string
```

(The built-in `set_state` action's helper `copy_string` in `builtin_actions.cpp`
shows this pattern.)

## Returning ActionStatus

Per the plan's Q12 action failure policy machine:

| Status | Engine behavior (entry/exit actions) | Engine behavior (continuous tick) |
|---|---|---|
| `OK` | Advance to next action; transition eval after all done | Continue continuous behavior |
| `PENDING` | Re-call same action next tick (escalates after ~1s) | Re-call next tick |
| `FAILED_RECOVERABLE` | Skip remaining actions in phase, continue with transitions | Deactivate ContinuousBehavior |
| `FAILED_ABORT` | Track → TRACK_FAILED, scenario aborts if main_track | Track → TRACK_FAILED |

Choose carefully:
- **Programming bug** (wrong param type, missing required param): `FAILED_ABORT`
  — this is a recipe author bug, fail loudly so they catch it during development
...
djb2_hash16 produces a uint16 — the birthday paradox says ~256 names before
...
1. ActionRegistry's `register_action` returns false (collision detected
   on insert)
2. compile_scenario.py also detects via the `tools/known_actions.json` lookup
   table (MVP: maintained manually — list all registered action names)

Resolution: rename one of the colliding actions. Convention: prefix with
the module name (`mc.set_target_temp` not `set_target_temp`) reduces collision
likelihood and aids diagnostics (you immediately see which module owns an
action).
...
private:
    ...
    /// Condition: true if |current - target| < tolerance (param "tol")
...
Add your registered names and computed hashes to `tools/known_actions.json`:
...
compile_scenario.py uses this table to validate recipes that reference your
action — without an entry, the recipe gets W0220 (unknown action), which `--strict`
elevates to an error in CI.
...
- `components/modesp_scenario/src/builtin_actions.cpp` — built-in actions
  source (good template for your own implementations)
