...
# Hand-authored 8×16 pictographs (same LSB-first/bit0=leftmost order as render_glyph_panel,
# so they drop straight into PANEL_FONT). Mapped to Private-Use codepoints 0x80+ — the panel
# module emits them as raw bytes inside the show_text string and the existing glyph-block loop
# renders them inline ("[thermo]29.7C"). NO protocol / ble_service.cpp change.
PANEL_ICONS = {
    0x80: [0x00,0x0C,0x14,0x14,0x14,0x1C,0x14,0x1C,0x14,0x1C,0x3E,0x2A,0x3E,0x3E,0x1C,0x00],  # thermometer
    0x81: [0x00,0x08,0x08,0x1C,0x1C,0x3E,0x3E,0x7F,0x7F,0x7F,0x7F,0x3E,0x3E,0x1C,0x00,0x00],  # droplet
    0x82: [0x00,0x1C,0x22,0x22,0x1C,0x00,0x08,0x3E,0x7F,0x7F,0x7F,0x7F,0x77,0x77,0x00,0x00],  # person
    0x83: [0x00,0x08,0x1C,0x1C,0x3E,0x3E,0x3E,0x7F,0x7F,0x7F,0xFF,0x00,0x1C,0x1C,0x00,0x00],  # alarm bell
    0x84: [0x00,0x00,0x00,0x3E,0x41,0x1C,0x22,0x08,0x1C,0x00,0x08,0x08,0x00,0x00,0x00,0x00],  # wifi
    0x85: [0x00,0x00,0x3C,0x42,0x4A,0x4A,0x7A,0x42,0x42,0x42,0x3C,0x00,0x00,0x00,0x00,0x00],  # clock
}
ICON_NAMES = {0x80: "thermometer", 0x81: "droplet", 0x82: "person", 0x83: "bell", 0x84: "wifi", 0x85: "clock"}
...
def generate_panel(args):
    ...
    icon_cps = list(PANEL_ICONS)               # pictograph icons appended AFTER °
    rows = []
    for cp in cps:
        rows += render_glyph_panel(chr(cp), font, args.x_off, args.y_off, args.threshold)
    for cp in icon_cps:
        rows += PANEL_ICONS[cp]                 # literal hand-authored bytes (not TTF-rendered)
    all_cps = cps + icon_cps
    count = len(all_cps)
    deg_index = len(cps) - 1                    # ° is the last of cps; icons follow at len(cps)+i
    ...
    out = [
        ...
        f"// + {len(icon_cps)} піктограм (Private-Use 0x80+): {', '.join(ICON_NAMES[c] for c in icon_cps)}.",
        "",
        "#include <cstdint>",
        "#include <cstddef>",
        "",
    ...
    for i, cp in enumerate(all_cps):
        ...
        if cp in PANEL_ICONS:
            label = f"icon {ICON_NAMES.get(cp, '?')}"
        else:
            label = f"'{chr(cp)}'" if 0x20 < cp < 0x7F else "' '"
        out.append(f"    {body},  // U+{cp:04X} {label}")
    out += [
        "};",
        "",
        ...
        f"    if (cp == 0x00B0)            return {deg_index};  // °",
    ]
    for i, cp in enumerate(icon_cps):
        out.append(f"    if (cp == 0x{cp:02X})            return {len(cps) + i};  // {ICON_NAMES.get(cp, 'icon')}")
    out += [
        "    return 0;  // space",
        "}",
        "",
    ...
    fp.write_text("\n".join(out), encoding="utf-8")
    print(f"  + {fp} ({count} glyphs = {len(cps)} text + {len(icon_cps)} icons, {PANEL_W}x{PANEL_H} LSB-first)")
    for cp in icon_cps:   # ASCII preview to eyeball the hand-authored pictographs
        print(f"  icon U+{cp:04X} {ICON_NAMES.get(cp, '')}:")
        for b in PANEL_ICONS[cp]:
            print("      " + "".join('#' if (b >> x) & 1 else '.' for x in range(PANEL_W)))
