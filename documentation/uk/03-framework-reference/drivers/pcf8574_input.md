...
Драйвер реєструється як `sensor` з `hardware_type: i2c_expander_input`, надає capability `binary_in` і має `multiple_per_bus: true`. Одна прив'язка на біт входу. Роль прив'язується за capability, а не за драйвером (R0.1 / R3.1) — модуль просить `binary_in`, не знаючи, хто його дає.
...
- Кожна прив'язка — окремий екземпляр драйвера: на кожному такті вона виконує **власне читання I2C** чипа (`read_state`) і виймає свій біт. Кілька прив'язок на одному чипі = кілька читань за цикл (≈6 читань за 100 мс на 100 кГц ≈ 1,8 мс сумарно — прийнятно).
...
`{"capability": "binary_in", "type": "bool"}` — поточний стан контакту (з опціональною інверсією), віддзеркалений у `equipment.<role>`.
...
- Демонструє паттерн `multiple_per_bus` — кілька екземплярів драйвера на одному чипі, кожен зі своїм читанням.
