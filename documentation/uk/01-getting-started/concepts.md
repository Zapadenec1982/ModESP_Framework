...
**Роль = здатність (capability), ніколи не драйвер** (R0.1). Модуль
оголошує роль через її **capability** — `temperature`, `relay_out`,
`humidity`, `dimmer` тощо — а НЕ через конкретний драйвер. Термостат
потребує «температуру» і не знає, хто її дає: `ds18b20`, NTC, BLE-канал
чи майбутній LoRa-сенсор. Здатності — і сенсорні, і актуаторні —
перелічені в `tools/capabilities.json` (єдине джерело словника). Роль
приймає канал драйвера ⟺ їхні `capability` рівні й напрям (in/out)
узгоджений — ніколи за іменем драйвера, `hw_type` чи транспортом (R3.1).

Наслідок: **джерело здатності замінне**. Дротовий драйвер, BLE-пристрій
чи майбутній транспорт (LoRa/MQTT/ESP-NOW) заповнюють ту саму роль без
правки модуля (R0.2). Ідентичність віддаленого пристрою (BLE MAC, adv-name,
topic) живе на рядку **пристрою** (`board.json` / runtime `devices.json`),
НІКОЛИ на прив'язці ролі (R0.3) — тож роль лишається транспорт-агностичною.
