...
> **Status:** Stage 0 (specifications + skeleton) — implementation in progress. Most files are placeholders that are filled out incrementally per implementation step. See the plan at `.claude/plans/quirky-imagining-lake.md`.

## Architectural docs

| File | Topic | Filled in Step |
|---|---|---|
...
| [usage/03_registering_actions.md](usage/03_registering_actions.md) | Custom actions in domain modules | 5 |
...
| [0003](adr/0003-tick-order-sync-semantics.md) | Tick-order cross-track sync (NOT snapshot) | placeholder, filled in Step 13 |
| [0004](adr/0004-recipe-as-manifest.md) | Recipe = manifest with `scenario` section | placeholder, filled in Step 2 |
| [0005](adr/0005-isa88-resource-arbitration.md) | ISA-88 §5.3 acquire-before-start | placeholder, filled in Step 10 |
| [0006](adr/0006-no-builtin-continuous-behaviors.md) | 0 built-in continuous behaviors in MVP | placeholder, filled in Step 6 |
| [0007](adr/0007-mandatory-phase-timeouts.md) | Mandatory per-phase timeouts | placeholder, filled in Step 8 |
...
These docs are NOT in the Stage 1 deliverable; they are planned for Stage 1.5 once real value emerges:
...
- `usage/examples/03_resource_contention.md`, `04_long_running_with_resume.md`, `05_irrigation_cycle.md` (recipe paper-piloted in Step 0.75)
...
6. ADRs `0001-0007` — critical decisions with rationale

For implementers:
1. Plan `.claude/plans/quirky-imagining-lake.md` — single source of truth for architecture
2. ADRs — prevent re-litigation of settled decisions
3. Architectural docs (01-10) — fill them out as code lands
