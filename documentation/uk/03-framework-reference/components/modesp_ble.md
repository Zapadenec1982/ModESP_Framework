...
Singleton, через який драйвери/модулі керують підключеним пристроєм. Драйвер `ble_led_panel` володіє транспортом і control-plane через `BlePanel`; модуль `panel` пише через нього контент — розв'язка йде саме через цей singleton.
...
`ble_led_panel` (actuator) — це **connect**-пристрій, що матчиться за **adv-name**, не за MAC. Драйвер володіє BLE-лінком і control-plane через `BlePanel` (живлення ON/OFF, яскравість 0..100 %), а **модуль** `panel` володіє відображуваним контентом. Декомпозиція йде через singleton `BlePanel`: драйвер відповідає за транспорт/керування, модуль — за «що показати».
