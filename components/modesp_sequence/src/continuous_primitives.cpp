...
bool read_state_float(modesp::SharedState* state, const char* key, float& out) {
    if (state == nullptr) return false;
    auto opt = state->get(modesp::StateKey(key));
    if (!opt.has_value()) return false;
    if (auto pf = etl::get_if<float>(&*opt))   { out = *pf; return true; }
    if (auto pi = etl::get_if<int32_t>(&*opt)) { out = static_cast<float>(*pi); return true; }
...
bool primitives::register_primitives() {
