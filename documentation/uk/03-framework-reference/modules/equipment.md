...
Equipment — це **сервісний модуль** (пріоритет `CRITICAL` = 0, стартує першим, зупиняється останнім). Framework-база (`EquipmentBase`) живе у `components/modesp_equipment/`; demo-сабклас (`EquipmentModule`) і маніфест — у `modules/equipment/`.
...
Зберігається (`persist: true`) — налаштування користувача переживає перезавантаження. WebUI показує його як уставку; підписники MQTT приймають записи для віддаленого налаштування. Калібрування самого сенсора (B-коефіцієнт, опори, зміщення) — це параметри драйвера у прив'язці, а не ключі стану equipment.
...
Список `mqtt.subscribe` дозволяє зовнішнім клієнтам налаштовувати параметри без перекомпіляції:

```bash
# Налаштування коефіцієнта фільтра через MQTT
...
`requires` у маніфесті Equipment оголошує **які ролі (здатності) очікує модуль**:
...
| `capability` | Здатність, яку роль споживає (`temperature`, `relay_out`…). Роль оголошує здатність, **ніколи не драйвер** — джерело (ds18b20 / NTC / BLE-канал / майбутній LoRa) замінне (R0.1 / R3.1). Словник — `capabilities.json`. |
| `label` | Назва у людиночитному форматі (для UI). Без транспорту (R1.3). |
| `optional` | Якщо `true`, відсутність прив'язки не зупиняє запуск. За замовчуванням `false`. |

Роль приймає канал ⟺ `capability` збігається і напрям (in/out) узгоджений — жодного драйвера чи транспорту в предикаті (R3.1). Який саме драйвер заповнить роль, вирішує `bindings.json` (це маршрутизація, не частина `requires`).
...
**Глобальний коефіцієнт фільтра:** `equipment.filter_coeff` спільний для всіх сенсорів модуля — це один параметр згладжування, а не per-role. Якщо різним сенсорам потрібне різне згладжування, розділіть їх на окремі equipment-сабкласи. Калібрування конкретного сенсора (B-коефіцієнт, зміщення) задається у драйвері прив'язки, а не тут.
