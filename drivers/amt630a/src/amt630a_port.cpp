...
void Amt630aPort::present_main(const MainView& view) {
    // Багата презентація idle-екрана — ЛЕГІТИМНИЙ вибір драйвера (ADR-002 §6: «презентацію
    // вирішує драйвер»; ADR-003 §3.4). CharGridLayout — ОПЦІЙНА утиліта (SSD1306 її не вживає),
    // тож власний layout у порті НЕ порушує шов: усе лишається в адаптері, не тече в модуль.
    // «Головне значення» несе ПОРЯДОК view.items (items[0]=головне) — чистий семантичний вхід.
    //
    // Перший пункт — ВЕЛИКИМ у вікні W0 ×2, решта — дрібними рядками у вікні W1 ×1. Кожне вікно
    // читає свою ділянку BGMAP (W0=0x000, W1=0x040), тому розміри незалежні. Кожен рядок добиваємо
    // RAM-пробілом (0x1C0) на всю ширину вікна — інакше старі тайли BGMAP лишаються «хвостом».
