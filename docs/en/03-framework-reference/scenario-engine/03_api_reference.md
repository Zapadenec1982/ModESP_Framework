...
19 values. Full table в [10_error_model.md](10_error_model.md#engineerror-codes).
...
struct SequenceRuntime {
    enum class State : uint8_t {
        IDLE = 0,        ///< slot empty
        LOADED,          ///< .modr loaded i validated, awaiting start()
        RUNNING,
        PAUSED,
        ABORTING,        ///< abort у progress; tracks running exit actions
                         ///< (per-phase $abort) або forced FAILED (scenario abort)
        COMPLETED,       ///< completion_rule satisfied
        FAILED,          ///< abort path completed або terminal failure
    };
};
...
Wire-format-equivalent із `modr_param_entry`. POD, exactly 8 bytes.
...
### `ActionContext` — passed до each action/condition handler
...
// Optional: NVS persistence callbacks (см. [07_persistence.md](07_persistence.md))
...
void on_stop() override;            // unloads все slots
```
...
`load_path` reads file via `std::fopen` (target: ESP-IDF VFS LittleFS).
...
`start` атомарно acquires scenario-scope resources. Returns `RESOURCE_CONTENDED`
on conflict із no partial state.
...
recipe authors використовують global transitions to dedicated cleanup phases.
...
`try_recover` requires slot already у LOADED state (recipe re-loaded after
boot). Calls read callback, deserializes token, applies state, sets
scenario state to PAUSED. See [07_persistence.md](07_persistence.md).
...
public:
    ...
    bool register_condition(const ActionDescriptor& d);
                            ///< Returns false on collision, capacity exhaust,
                            ///< або hash != djb2_hash16(name) inconsistency.
...
Capacity: `MAX_REGISTRY_ENTRIES = 64` per registry (actions і conditions
separate). See [usage/03_registering_actions.md](usage/03_registering_actions.md).

## `ContinuousRegistry` — singleton (Stage 1.5)

Source: `continuous_behavior.h`. Reserved для future ContinuousBehavior
implementations (PID, hysteresis, ramp). 0 built-ins у Stage 1.
...
Composite conditions (`all_of`, `any_of`, `not`) handled inline by engine
без registry entries — use sentinel hashes у `cond_pool` entries.

## `LoadedScenario` — read-only view (advanced)

Source: `modr_loader.h`. Returned by `modr_validate` on success. Holds
raw pointers into caller-owned buffer.
...
See [06_resource_arbitration.md](06_resource_arbitration.md) для design.

## Cross-references

- [usage/01_quickstart.md](usage/01_quickstart.md) — typical use pattern
- [usage/03_registering_actions.md](usage/03_registering_actions.md) — extending із custom actions
- [02_binary_format.md](02_binary_format.md) — `.modr` byte layout (input до loader)
- [07_persistence.md](07_persistence.md) — NVS callback contract і token format
- [10_error_model.md](10_error_model.md) — full EngineError table з handling guidance
