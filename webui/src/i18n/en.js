export default {
  'app.loading': 'Loading...',
  'app.error': 'Connection Error',
  'app.retry': 'Retry',
  'status.online': 'Online',
  'status.offline': 'Offline',
  'alarm.banner': 'ALARM',
  'eq.on': 'ON',
  'eq.off': 'OFF',
  'chart.loading': 'Loading...',
  'chart.no_data': 'No data',
  'chart.events': 'Events',
  'chart.title': 'Temperature',
  'event.7': 'Alarm cleared',
  'event.10': 'Power on',
  'bind.status': 'Equipment Status',
  'bind.loading': 'Loading...',
  'bind.saved': 'Saved. Restart required.',
  'bind.saved_title': 'Saved',
  'bind.saved_msg': 'Restart the controller to apply changes.',
  'bind.restart': 'Restart',
  'bind.later': 'Later',
  'bind.required': 'Required',
  'bind.optional': 'Optional',
  'bind.save': 'Save',
  'bind.saving': 'Saving...',
  'bind.scan': 'Scan Bus',
  'bind.scanning': 'Scanning...',
  'bind.sensors': 'Sensors',
  'bind.actuators': 'Actuators',
  'bind.add_equip': 'Add Equipment',
  'bind.used': 'in use',
  'bind.choose_hw': 'Choose hardware',
  'bind.choose_channel': 'Choose channel',
  'bind.pick': 'Select',
  'bind.selected': 'selected',
  'bind.no_devices': 'No devices found',

  // Devices (remote device subscriptions)
  'dev.loading': 'Loading…',
  'dev.load_failed': 'Failed to load devices',
  'dev.subscribed': 'Subscribed devices',
  'dev.none': 'No subscribed devices yet. Scan below to add one.',
  'dev.scan': 'Scan',
  'dev.scanning': 'Scanning…',
  'dev.scan_title': 'Find BLE devices',
  'dev.scan_desc': 'Only devices our drivers understand are listed. Tell same-type sensors apart by their readings (tilt/bring the right one closer) or by RSSI.',
  'dev.scan_hint': 'Press “Scan” to find nearby devices',
  'dev.none_found': 'No supported devices found nearby',
  'dev.subscribe': 'Subscribe',
  'dev.have': 'subscribed',
  'dev.save': 'Save',
  'dev.saving': 'Saving…',
  'dev.unsaved': 'Unsaved changes',
  'dev.saved_title': 'Devices saved',
  'dev.saved_msg': 'Changes take effect after a restart.',

  'page.not_found': 'Page not found',
  'conn.lost': 'Connection lost. Reconnecting...',
  'conn.retry': 'Reconnect',
  'conn.restored': 'Connected',
  'alert.saved': 'Saved!',
  'alert.error': 'Save error',
  'alert.confirm_ota': 'Update firmware? Device will restart.',
  'alert.only_bin': 'Only .bin files',
  'ota.uploading': 'Uploading...',
  'ota.done': 'Done! Restarting...',
  'ota.upload': 'Update firmware',
  'ota.select': 'Select firmware file',
  'wifi.scan': 'Scan',
  'wifi.scanning': 'Scanning...',
  'tz.label': 'Timezone',
  'pass.show': 'Show',
  'pass.hide': 'Hide',
  'auth.title': 'Authorization',
  'auth.user': 'Username',
  'auth.pass': 'Password',
  'auth.login': 'Login',
  'auth.error': 'Invalid credentials',
  'nav.more': 'More',
  'btn.save': 'Save',
  'btn.error': 'Error',
  'btn.remove': 'Remove',
  'eq.filter': 'Digital filter',
  'eq.offset': 'Offset °C',
  'chart.min_ago': 'min ago',
};
