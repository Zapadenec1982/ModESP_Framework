...
`{"capability": "relay_out", "type": "bool"}` — скомандований рівень на прив'язаному виводі розширювача, віддзеркалений у `equipment.<role>`. Роль споживача оголошує здатність `relay_out`, а не цей драйвер; будь-який актуатор із тією ж capability замінний (R0.1, R3.1).
