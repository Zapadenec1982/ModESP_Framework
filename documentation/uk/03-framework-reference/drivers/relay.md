...
Драйвер реєструється як `actuator`, надає capability `relay_out` (`hardware_type: gpio_output`, `requires_address: false`). Роль оголошує саме capability `relay_out`, а не цей драйвер (R0.1/R3.1) — джерело сигналу замінне. Один актуатор на прив'язаний GPIO.
...
`{"capability": "relay_out", "type": "bool"}` — поточний скомандований стан, віддзеркалений у `equipment.<role>`. Роль з capability `relay_out` (напрям out) матчиться до цього драйвера (R3.1).
